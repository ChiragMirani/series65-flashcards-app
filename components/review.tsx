...
import type { Card,Rating } from '@/lib/model';
...
export function Review(){
  ...
  if(!session)return <div className="page narrow empty-state"><h1>Ready for a short session?</h1><p>Due cards come first, followed by new cards within your daily limits.</p><button className="button primary" disabled={busy} onClick={()=>void dispatch({type:'start',now:new Date().toISOString()})}>Start review</button></div>;
...
function ReviewCard({card}:{card:Card}){
  ...
  const rate=useCallback(async(rating:Rating)=>{if(!revealed||busy)return;await dispatch({type:'rate',cardId:card.id,sessionId:session.id,rating,now:new Date().toISOString()});},[revealed,busy,dispatch,card.id,session.id]);
  ...
  return <div className="page review-page"><div className="review-top"><Link href="/"><ArrowLeft aria-hidden="true"/>Save & exit</Link><span>{session.completed.length} of {session.initialCount} complete</span><span>{session.queue.length} remaining</span></div><div className="meter session-meter" role="progressbar" aria-label="Session completion" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100}><span style={{width:`${progress}%`}}/></div><div className="review-meta"><span>Section {card.section} · {card.sectionTitle}</span><span className="badge">{card.type}</span></div>
