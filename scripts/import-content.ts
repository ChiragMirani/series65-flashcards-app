...
for(const section of sections){
  ...
  for(const line of (text[section.number]||'').trim().split('\n')){
    ...
    const add=(suffix:string,type:Card['type'],front:string,answer:string,explanation:string)=>{
      ...
      const fields={...common,id:`${ruleId}-${suffix}`,type,front:expanded[0],answer:expanded[1],governingRule:expanded[2],trap:expanded[3],explanation:expanded[4],...(expanded[5]?{memoryHook:expanded[5]}:{})};
