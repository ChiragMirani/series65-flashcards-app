...
for(const section of sections){
  ...
  for(const line of (text[section.number]||'').trim().split('\n')){
    ...
    const add=(suffix:string,type:Card['type'],front:string,answer:string,explanation:string)=>{
      const expanded=expandSequence([front,answer,rule,trap,explanation]);
      const fields={...common,id:`${ruleId}-${suffix}`,type,front:expanded[0],answer:expanded[1],governingRule:expanded[2],trap:expanded[3],explanation:expanded[4]};
      cards.push(cardSchema.parse({...fields,contentVersion:hash(JSON.stringify(fields)+section.body)}));
    };
    add('recall','recall',prompt,rule,`Distinguish the tested condition from this misconception: “${trap}”`);
    if(priority==='high'||retest) add('contrast','contrast',`A trainee makes this claim: “${trap}” What correction is needed?`,rule,`The mistaken claim substitutes a neighboring rule or omits a required condition. ${rule}`);
...
const manifest={schemaVersion:1,sourceRevision:'2026-09-12',sourceFiles:sources.map(s=>({file:s.file,sha256:hash(s.text)})),rules:ledger.length,cards:cards.length,sections:sections.map(s=>({number:s.number,title:s.title,sha256:hash(s.body)}))};
const outputs:Record<string,string>={'content/deck.json':JSON.stringify(cards,null,2)+'\n','content/source-manifest.json':JSON.stringify(manifest,null,2)+'\n','docs/deck-coverage.md':coverage,'docs/content-provenance.md':provenance};
