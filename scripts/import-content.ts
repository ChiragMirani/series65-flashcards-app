...
Object.assign(glossary,{TIPS:'Treasury Inflation-Protected Securities',ETN:'exchange-traded note',ETF:'exchange-traded fund'});
...
const cards:Card[]=[];
const ledger:{id:string;section:number;locator:string;category:Category;count:number;sourcePath:string;sourceHeading:string}[]=[];
...
for(const section of sections){
  ...
  for(const line of (text[section.number]||'').trim().split('\n')){
    ...
    ledger.push({id:ruleId,section:section.number,locator,category:specialCategory,count:1+Number(priority==='high'||retest)+extras.length,sourcePath:path.join(sourceDir,sourceFile),sourceHeading:common.sourceHeading});
  }
}
// The user's additional scope is authored separately: never pretend these rules
// were extracted from an unchanged canonical section or modify the source files.
const expansionFile='content/review-expansion.ts';
for(const seed of reviewRules) {
  const section=sections.find(s=>s.number===seed.section);
  const point=reviewChecklist.find(p=>p.number===seed.point);
  if(!section||!point) throw new Error(`Invalid review-rule location: ${seed.key}`);
  const ruleId=`s65-${String(seed.section).padStart(2,'0')}-${seed.key}`;
  if(!point.rules.some(id=>id===ruleId)) throw new Error(`Review rule not mapped to its checkpoint: ${ruleId}`);
  const category:Category=seed.section<=11?'laws':seed.section<=17?'recommendations':seed.section<=21?'vehicles':'economics';
  const sourceHeading=`Review point ${point.number} — ${point.title}`;
  const add=(suffix:string,type:Card['type'],front:string,answer:string,explanation:string)=>{
    const expanded=expandSequence([front,answer,seed.answer,seed.trap,explanation,seed.memoryHook||'']);
    const fields={id:`${ruleId}-${suffix}`,ruleId,deckId:'series65',section:seed.section,sectionTitle:section.title,category,priority:'high',type,
      front:expanded[0],answer:expanded[1],governingRule:expanded[2],trap:expanded[3],explanation:expanded[4],...(expanded[5]?{memoryHook:expanded[5]}:{}),
      tags:[seed.key,'essential-review',`review-point-${seed.point}`,'retest'],sourcePath:expansionFile,sourceHeading,
      officialSources:sourcesForRule(seed.section,seed.key),retest:true,reviewStatus:'draft'};
  ...
  ledger.push({id:ruleId,section:seed.section,locator:`Supplement: review point ${seed.point}`,category,count:2+Number(!!seed.application),sourcePath:path.resolve(expansionFile),sourceHeading});
}
const nasaaFile='content/nasaa-supplement.ts';
for(const seed of nasaaRules) {
  assertNasaaSource(seed.source,seed.clause);
  const section=sections.find(s=>s.number===seed.section);
  if(!section) throw new Error(`Invalid NASAA supplement section: ${seed.key}`);
  const category:Category=seed.section<=11?'laws':seed.section<=17?'recommendations':seed.section<=21?'vehicles':'economics';
  const ruleId=`s65-${String(seed.section).padStart(2,'0')}-${seed.key}`;
  const sourceHeading=`NASAA supplement — ${seed.clause}`;
  for(const [suffix,front,answer] of [['recall',seed.front,seed.answer],['contrast',seed.contrast.front,seed.contrast.answer]]) {
    const expanded=expandSequence([front,answer,seed.answer,seed.trap,seed.explanation]);
    const fields={id:`${ruleId}-${suffix}`,ruleId,deckId:'series65',section:seed.section,sectionTitle:section.title,category,
      priority:category==='laws'?'high':'medium',type:suffix,front:expanded[0],answer:expanded[1],governingRule:expanded[2],trap:expanded[3],explanation:expanded[4],
      tags:[seed.key,'nasaa-supplement','retest'],sourcePath:nasaaFile,sourceHeading,officialSources:[seed.source],retest:true,reviewStatus:'draft'};
    cards.push(cardSchema.parse({...fields,contentVersion:hash(JSON.stringify(fields))}));
  }
...
let checklistReport='# Requested 30-topic content coverage\n\nExpanded September 13, 2026. Each requested topic has recall plus original application, calculation, or contrast practice. Related boundary cases are split into separate objectives. The list maps to live deck IDs, not planned work. All cards remain draft pending human accuracy and provenance review. The screen and study interaction are unchanged.\n\n';
if(reviewChecklist.length!==30||new Set(reviewChecklist.map(p=>p.number)).size!==30) throw new Error('Expected 30 unique review checkpoints.');
for(const point of reviewChecklist) {
  const subset=cards.filter(c=>point.rules.some(id=>id===c.ruleId));
  for(const id of point.rules) if(!subset.some(c=>c.ruleId===id)) throw new Error(`Missing requested rule: ${id}`);
  if(!subset.some(c=>c.type==='recall')||!subset.some(c=>c.type!=='recall')) throw new Error(`Incomplete learning sequence: ${point.number}`);
  checklistReport+=`## ${point.number}. ${point.title}\n\n${subset.length} cards covering ${point.rules.length} objectives.\n\n| Card ID | Type | Question | Answer |\n|---|---|---|---|\n`+subset.map(c=>`| ${c.id} | ${c.type} | ${cell(c.front)} | ${cell(c.answer)} |`).join('\n')+'\n\n';
}
coverage+=`\n## Requested expansion\n\n${reviewRules.length} additional objectives supplement the canonical sheet, grouped into the nearest existing subject sections. Their provenance points to content/review-expansion.ts rather than the original private file. See [30-topic coverage](requested-review-coverage.md) for the exact question/answer pairs covering all requested topics.\n`;
coverage+=`\n## NASAA-supported supplement\n\n${nasaaRules.length} additional objectives (${nasaaRules.length*2} cards) use independently authored wording and specifically inspected NASAA provisions. See [supplement ledger](nasaa-supplement.md). These are additions to the canonical sheet, not newly discovered source-file text.\n`;
const nasaaReport='# NASAA-supported additional concepts\n\nGenerated by npm run content:import. These independently authored pairs address the September 13, 2026 notes only where a precise NASAA provision was inspected. Every card remains draft. Model rules are not proof of adoption in every state. Host/type validation cannot establish substantive accuracy. The prior 703 cards retain their existing provenance and are not claimed to be NASAA-only.\n\n| Rule | Section | Provision | Direct NASAA source | Cards |\n|---|---:|---|---|---:|\n'+nasaaRules.map(r=>`| s65-${String(r.section).padStart(2,'0')}-${r.key} | ${r.section} | ${r.clause} | [${cell(r.source.title)}](${r.source.url}) | 2 |`).join('\n')+'\n';
const outputs:Record<string,string>={'content/deck.json':JSON.stringify(cards,null,2)+'\n','content/source-manifest.json':JSON.stringify(manifest,null,2)+'\n','docs/deck-coverage.md':coverage,'docs/content-provenance.md':provenance,'docs/answer-continuity.md':answerReview,'docs/public-references.md':publicReferences,'docs/requested-review-coverage.md':checklistReport,'docs/nasaa-supplement.md':nasaaReport};
