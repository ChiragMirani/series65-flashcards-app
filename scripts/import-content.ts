...
const authoredSources=await Promise.all([expansionFile,'content/review-checklist.ts'].map(async file=>({file,sha256:hash(await readFile(file,'utf8'))})));
...
const outputs:Record<string,string>={'content/deck.json':JSON.stringify(cards,null,2)+'\n','content/source-manifest.json':JSON.stringify(manifest,null,2)+'\n','docs/deck-coverage.md':coverage,'docs/content-provenance.md':provenance,'docs/answer-continuity.md':answerReview,'docs/public-references.md':publicReferences,'docs/requested-review-coverage.md':checklistReport};
