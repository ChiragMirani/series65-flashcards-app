...
test('one page opens a full deck and switches all subjects inline with the correct counter', async ({ page }) => {
  ...
  await expect(counter).toHaveText('1 of 1003');
  ...
  for (const [category, count] of Object.entries({ laws: 448, recommendations: 253, vehicles: 194, economics: 108 })) {
  ...
  await expect(counter).toHaveText('2 of 108');
...
test('hosted worker caches unvisited pages and saves phone reviews offline', async ({ page, context }) => {
  ...
  await expect(page.locator('.card-position')).toHaveText('1 of 1003');
  await page.getByRole('button', { name: 'Reveal answer', exact: true }).click();
  await page.getByRole('button', { name: 'Next card', exact: true }).click();
  await expect(page.locator('.question-button')).toBeVisible();
  ...
  await expect(page.locator('.card-position')).toHaveText('2 of 1003');
  await page.getByRole('link', { name: 'Progress', exact: true }).click();
  await page.getByRole('link', { name: 'Study', exact: true }).click();await expect(page.locator('.card-position')).toHaveText('2 of 1003');
});
...
test('repeated finger taps reveal and advance at the same spot without ratings', async ({ page }) => {
  ...
  await expect(page.locator('.card-position')).toHaveText('4 of 1003');
  await page.getByText('Card details', { exact: true }).tap();
  await page.locator('.answer-text').tap();
  await expect(page.locator('.card-position')).toHaveText('5 of 1003');
  await expect(page.locator('.question-button')).toBeVisible();
  const next = await page.locator('.study-question').innerText();
  await page.reload();
  await expect(page.locator('.study-question')).toHaveText(next);
...
test('VocabDeck reveal motion, order toggle, previous card and share link', async ({ page, browser }) => {
  ...
  await page.locator('.study-face').tap();
  await expect(page.locator('.card-position')).toHaveText('2 of 108');
  ...
  try {
    ...
    await expect(friend.locator('.card-position')).toHaveText('1 of 108');
